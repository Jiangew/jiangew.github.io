...
- [1. RPC 调用安全策略](#1-rpc-调用安全策略)
    - [1.1 严峻的安全形势](#11-严峻的安全形势)
    - [1.2 敏感数据加密传输](#12-敏感数据加密传输)
        - [1.2.1 基于 SSL/TLS 的通道加密](#121-基于-ssltls-的通道加密)
        - [1.2.2 针对敏感数据的单独加密](#122-针对敏感数据的单独加密)
    - [1.3 认证和鉴权](#13-认证和鉴权)
        - [1.3.1 身份认证](#131-身份认证)
        - [1.3.2 权限管控](#132-权限管控)
    - [1.4 数据完整性和一致性](#14-数据完整性和一致性)
- [2. gRPC 安全机制](#2-grpc-安全机制)
    - [2.1 SSL/TLS 认证](#21-ssltls-认证)
        - [2.1.1 SSL/TLS 工作原理](#211-ssltls-工作原理)
        - [2.1.2 HTTP/2 的 ALPN](#212-http2-的-alpn)
        - [2.1.3 gRPC 的 TLS 策略](#213-grpc-的-tls-策略)
        - [2.1.4 基于 TLS 的 gRPC 代码示例](#214-基于-tls-的-grpc-代码示例)
        - [2.1.5 gRPC TLS 源码分析](#215-grpc-tls-源码分析)
    - [2.2 Google OAuth 2.0](#22-google-oauth-20)
        - [2.2.1 工作原理](#221-工作原理)
        - [2.2.2 代码示例](#222-代码示例)
    - [2.3 自定义安全认证策略](#23-自定义安全认证策略)
...
* <strong>个人 / 企业敏感数据加密：</strong>例如针对个人的账号、密码、手机号等敏感信息进行加密传输，打印接口日志时需要做数据模糊化处理等，不能明文打印；
* <strong>对调用方的身份认证：</strong>调用来源是否合法，是否有访问某个资源的权限，防止越权访问；
* <strong>数据防篡改和完整性：</strong>通过对请求参数、消息头和消息体做签名，防止请求消息在传输过程中被非法篡改。
...
使用 SSL/TLS 的典型场景如下所示：<br>
![](/assets/images/post/20181020/grpc-05-01.png)<br>
...
它的工作原理如下所示：<br>
![](/assets/images/post/20181020/grpc-05-02.png)<br>
...
### 1.3 认证和鉴权
RPC 的认证和鉴权机制主要包含两点：
* <strong>认证：</strong>对调用方身份进行识别，防止非法调用；
* <strong>鉴权：</strong>对调用方的权限进行校验，防止越权调用。
...
身份认证的方式较多，例如 HTTP Basic Authentication、OAuth2 等，比较简单使用的是令牌认证（Token）机制，它的工作原理如下所示：<br>
![](/assets/images/post/20181020/grpc-05-03.png)<br>
...
在 RPC 调用领域比较流行的是基于 OAuth2.0 的权限认证机制，它的工作原理如下：<br>
![](/assets/images/post/20181020/grpc-05-04.png)<br>
...
* <strong>通道凭证：</strong>默认提供了基于 HTTP/2 的 TLS，对客户端和服务端交换的所有数据进行加密传输；
* <strong>调用凭证：</strong>被附加在每次 RPC 调用上，通过 Credentials 将认证信息附加到消息头中，由服务端做授权认证；
* <strong>组合凭证：</strong>将一个频道凭证和一个调用凭证关联起来创建一个新的频道凭证，在这个频道上的每次调用会发送组合的调用凭证来作为授权数据，最典型的场景就是使用 HTTP S 来传输 Access Token；
* <strong>Google 的 OAuth 2.0：</strong>gRPC 内置的谷歌的 OAuth 2.0 认证机制，通过 gRPC 访问 Google API 时，使用 Service Accounts 密钥作为凭证获取授权令牌。

### 2.1 SSL/TLS 认证
gRPC 基于 HTTP/2 协议，默认会开启 SSL/TLS，考虑到兼容性和适用范围，gRPC 提供了三种协商机制：
* <strong>PlaintextNegotiator：</strong>非 SSL/TLS 加密传输的 HTTP/2 通道，不支持客户端通过 HTTP/1.1 的 Upgrade 升级到 HTTP/2, 代码示例如下（PlaintextNegotiator 类）：
```java
...
* <strong>PlaintextUpgradeNegotiator：</strong>非 SSL/TLS 加密传输的 HTTP/2 通道，支持客户端通过 HTTP/1.1 的 Upgrade 升级到 HTTP/2，代码示例如下（PlaintextUpgradeNegotiator 类）：
```java
...
<strong>TlsNegotiator：</strong>基于 SSL/TLS 加密传输的 HTTP/2 通道，代码示例如下（TlsNegotiator 类）：
```java
...
SSL 单向认证的流程图如下所示：<br>
![](/assets/images/post/20181020/grpc-05-05.png)<br>

SSL 双向认证相比单向认证，多了一步服务端发送认证请求消息给客户端，客户端发送自签名证书给服务端进行安全认证的过程。

客户端接收到服务端要求客户端认证的请求消息之后，发送自己的证书信息给服务端，信息如下：<br>
![](/assets/images/post/20181020/grpc-05-06.png)<br>

服务端对客户端的自签名证书进行认证，信息如下：<br>
![](/assets/images/post/20181020/grpc-05-07.png)<br>
...
以上代码示例来源：[链接](http://www.eclipse.org/jetty/documentation/9.3.x/alpn-chapter.html)

需要指出的是，Jetty ALPN 类库版本与 JDK 版本是配套使用的，配套关系如下所示：<br>
![](/assets/images/post/20181020/grpc-05-08.png)<br>

可以通过如下网站查询双方的配套关系：[链接](http://www.eclipse.org/jetty/documentation/9.3.x/alpn-chapter.html)
...
在启动参数中增加 SSL 握手日志打印以及 Jetty 的 ALPN Agent 类库，示例如下：<br>
![](/assets/images/post/20181020/grpc-05-09.png)<br>

启动服务端，显示 SSL 证书已经成功加载：<br>
![](/assets/images/post/20181020/grpc-05-10.png)<br>
...
![](/assets/images/post/20181020/grpc-05-11.png)<br>
...
SSL 的消息读取实际就是 ByteToMessageDecoder 将接收到的 SSL 加密后的报文解码为原始报文，然后将整包消息投递给后续的消息解码器，对消息做二次解码。基于 SSL 的消息解码模型如下：<br>
![](/assets/images/post/20181020/grpc-05-12.png)<br>
...
#### 2.2.2 代码示例
以 OAuth2 认证为例，客户端代码如下所示，创建 OAuth2Credentials，并实现 Token 刷新接口：<br>
![](/assets/images/post/20181020/grpc-05-13.png)<br>
...
### 2.3 自定义安全认证策略
参考 Google 内置的 Credentials 实现类，实现自定义的 Credentials，可以扩展 gRPC 的鉴权策略，Credentials 的实现类如下所示：<br>
![](/assets/images/post/20181020/grpc-05-14.png)<br>
...
[阅读原文](https://time.geekbang.org/column/article/4399)
